...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def neighbours: List[(Block, Move)] = List((up, Up), ( down, Down), (right, Right), (left,Left))

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbours: List[(Block, Move)] = 
      // filters according to whether first being BLock has a legal position in the terrain
      neighbours filter(_._1.isLegal)

    /**
     * Returns `true` if the block is standing.
     */
    // checks each block is on the same coordinate
    def isStanding: Boolean = 
      // checks if two blocks have same position. If they do returns true
      b1 == b2
     ...
     */
    def isLegal: Boolean = 
      // passes both blocks into terrain to see if they have a valid position
